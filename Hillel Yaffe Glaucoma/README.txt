...
glaucoma detection from retinal fundus images, developed as part of the
IDSC (International Data Science Competition) project "Dawnbringer",
...
by automating the detection process using deep learning on fundus photography
— a widely available and non-invasive imaging modality.
...
- Trains a Convolutional Neural Network (CNN) / transfer learning model to
...
- Generates visual explanations (e.g., Grad-CAM) to highlight regions
  of interest that influenced the model's decision

POTENTIAL MEDICAL APPLICATIONS
-------------------------------
- Automated screening tools for glaucoma in primary care clinics and
  rural/remote areas where ophthalmologists are unavailable
...
1. Open the notebook in Jupyter Lab or Google Colab
   (Google Colab recommended due to large image dataset and GPU requirements)
...
berbasis AI dari citra fundus retina, yang dikembangkan sebagai bagian dari
proyek IDSC (International Data Science Competition) "Dawnbringer", dengan
menggunakan dataset dari Hillel Yaffe Medical Center.

Glaukoma adalah salah satu penyebab utama kebutaan permanen di seluruh dunia.
Deteksi dini sangat penting, namun membutuhkan keahlian spesialis dan
peralatan yang mahal. Proyek ini bertujuan untuk mendemokratisasi diagnosis
dini dengan mengotomatisasi proses deteksi menggunakan deep learning pada
foto fundus — modalitas pencitraan yang tersedia luas dan non-invasif.
...
- Melatih model Convolutional Neural Network (CNN) / transfer learning untuk
...
- Menghasilkan penjelasan visual (contoh: Grad-CAM) untuk menyoroti area
  yang mempengaruhi keputusan model

POTENSI APLIKASI MEDIS
-----------------------
- Alat skrining glaukoma otomatis di klinik perawatan primer dan daerah
  terpencil yang kekurangan dokter spesialis mata
- Alat pendukung opini kedua bagi dokter spesialis mata (ophthalmologis)
- Program skrining skala populasi menggunakan platform telemedicine
- Integrasi dengan sistem kamera fundus yang sudah ada untuk diagnosis
  di titik perawatan (point-of-care)
...
1. Buka notebook di Jupyter Lab atau Google Colab
   (Google Colab direkomendasikan karena dataset gambar yang besar dan
   kebutuhan GPU)
...
dimaksudkan untuk diagnosis klinis atau untuk menggantikan penilaian dokter
